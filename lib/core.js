...
try {
  ...
  logger.error("Failed to load configuration files:", error.message);
  process.exit(1);
}
...
class MQTTtoModbusSDK extends EventEmitter {
  ...
  startMQTT() {
    if (this.isMQTTConnected) {
      logger.error("MQTT is already connected.");
  ...
  _setHoldingRegister(addr, value, source = "external") {
    if (source === "external") {
      ...
      logger.info(`external set ${addr} to ${value} from Modbus`);
      console.log(`external set ${addr} to ${value} from Modbus`);
    } else if (source === "internal") {
      // 內部修改邏輯
      logger.info(`internal set ${addr} to ${value} from MQTT`);
