...
async function loadLogs() {
  ...
  try {
    ...
    if (!data.lines.length) {
      viewer.textContent = "（Log 檔案為空或尚未產生）";
    } else {
      viewer.textContent = data.lines.join("\n");
      viewer.scrollTop = viewer.scrollHeight;
    }
    statusEl.textContent = `顯示 ${data.lines.length} 行`;
  } catch (err) {
    viewer.textContent = `載入失敗：${err.message}`;
...
async function loadAudit() {
  const head = document.getElementById("audit-table-head");
  const body = document.getElementById("audit-table-body");
  const statusEl = document.getElementById("audit-status");
  const kind = document.getElementById("audit-kind-select").value;
  const lines = document.getElementById("audit-lines-select").value;
  const topic = document.getElementById("audit-topic-filter").value.trim();
  const channel = document.getElementById("audit-channel-filter").value.trim();
  statusEl.textContent = "載入中…";
  try {
    ...
    const cols = data.records.length ? Object.keys(data.records[0]) : [];
    head.innerHTML = cols.map(c => `<th style="text-align:left;padding:4px 8px;border-bottom:1px solid #444;">${c}</th>`).join("");
    body.innerHTML = data.records.map(r =>
      `<tr>${cols.map(c => `<td style="padding:4px 8px;border-bottom:1px solid #333;">${r[c] ?? ""}</td>`).join("")}</tr>`
    ).join("");
    statusEl.textContent = data.records.length ? `顯示 ${data.records.length} 筆` : "（無紀錄）";
  } catch (err) {
    body.innerHTML = "";
